...
int letterCounter(string s);
int moreLetterWord(int array[],int n);
int main() {
    string word;
    int num,x=0;
    int Save_Letter_Cout[num];
    cin >> num;
    for(int i=0;i<num;i++){
        cin>>word;
        Save_Letter_Cout[i] = letterCounter(word);
    }
    moreLetterWord(Save_Letter_Cout,num);
    return 0;
}
int letterCounter(string s){
    int letterCountArray[s.size()];
    int step=0;
    map<char,int> charCount;
    for (unsigned int i=0; i<s.size(); i++)
        charCount[s[i]]++;
    for (map<char, int>::iterator it = charCount.begin(); it != charCount.end(); ++it){
        letterCountArray[step] = it->second;
        step++;
    }
    return step;
}
...
// #define pr(i) cout <<int num; setprecision(i);
